...
static QXWindow* openwindow(int posx,int posy,int w,int h,char*winName)
{
    ...
    Window*dummy;
    int dummy_width,dummy_height,dummy_borderwidth,dummy_depth;
    XGetGeometry(X.d,qx->mywin,dummy, &qx->x, &qx->y, &dummy_width, &dummy_height, &dummy_borderwidth, &dummy_depth);
    if(noborder<=0)
        qx->y += 16;
...
static void gfxwindow_on(gfxwindow_t*win)
{
    ...
    i->qx=openwindow(200, 64, win->width, win->height, i->name);
    if(!i->qx)
	return;
    i->currentscr2=0;
    if(i->qx->bypp != 4) {
...
void gfxwindow_screenshot(gfxwindow_t*win, char*filename)
{
    int x,y;
    unsigned char*data2 = malloc(win->width*win->height*4);
    for(y=0;y<win->height;y++)
    for(x=0;x<win->width;x++) {
        unsigned char r = win->currentscr[y*win->width*4+x*4+2];
        unsigned char g = win->currentscr[y*win->width*4+x*4+1];
        unsigned char b = win->currentscr[y*win->width*4+x*4+0];
        unsigned char a = 255;
        data2[y*win->width*4+x*4+0] = a;
        data2[y*win->width*4+x*4+1] = r;
